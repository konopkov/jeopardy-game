...
function recursiveURLDecode(value) {
  if (isString(value)) {
    ...
    for (let key of Object.keys(value)) {
      // @ts-ignore
      decodedObject[specialDecodeURIComponent(key)] = recursiveURLDecode(
        value[key]
      );
    }
...
// @ts-ignore
export default function cleanupEvent(evt, options) {
  const event = evt || {};
  ...
  if (event.version === "2.0") {
    ...
    event.path = removeBasePath(
      event.requestPath || event.path,
      options.basePath
    );
  }
